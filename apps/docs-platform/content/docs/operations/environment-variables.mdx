...
| `ALLOWED_ORIGINS`         | No       | Comma-separated list of permitted origins for the API Proxy.                   | `https://api.veriworkly.com`   |
...
| `AUTH_COOKIE_DOMAIN`        | The domain scope for authentication cookies.          | `veriworkly.com`        |
| `AUTH_SESSION_TTL_SECONDS`  | Maximum session duration (seconds).                   | `2592000` (30 days)     |
| `AUTH_OTP_TTL_SECONDS`      | Validity duration for Email OTP tokens (seconds).     | `300` (5 minutes)       |
