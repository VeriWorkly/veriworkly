    ...
    <main className="space-y-5">
      <header className="border-border bg-card rounded-2xl border p-5 sm:p-6">
        ...
        <p className="text-muted mt-2 max-w-2xl text-sm leading-6">
          Every successful AI action appears here with its exact cost and resulting balance.
        </p>
      </header>
      ...
      <section className="border-border bg-card rounded-2xl border p-5">
        ...
        <p className="text-muted mt-2 max-w-3xl text-sm leading-6">
          Credits use fixed server-side costs per successful action. Failed actions do not consume
          credits. Older and sooner-expiring credits are used first.
        </p>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {Object.entries(economics?.actions ?? {}).map(([action, policy]) => (
            <div className="bg-background rounded-xl p-4" key={action}>
              <p className="text-sm font-bold">{action.replaceAll("_", " ")}</p>
              <p className="text-accent mt-2 text-xl font-black">
                {policy.costs.standard} / {policy.costs.expert} credits
              </p>
              <p className="text-muted mt-1 text-xs">Standard / Expert</p>
            </div>
          ))}
        </div>
        <p className="text-muted mt-4 text-xs">
          {wallet?.nextExpiryCredits
            ? `${wallet.nextExpiryCredits} credits expire ${new Date(wallet.nextExpiryAt!).toLocaleDateString()}.`
      ...
      <section className="border-border bg-card rounded-2xl border p-5">
        ...
        <div className="border-border mt-4 divide-y border-y">
          {history.length ? (
              ...
              >
                <div>
                  <p className="font-bold">
                    {item.action?.replaceAll("_", " ") || item.type.toLowerCase()}
                  </p>
                ...
                <p
                  className={
                    item.amount > 0 ? "font-black text-emerald-600" : "font-black text-rose-600"
                  }
                >
                  {item.amount > 0 ? "+" : ""}
                  {item.amount}
            ...
            <p className="text-muted py-5 text-sm">
              No credit usage yet. Successful AI actions will appear here.
            </p>
          )}
        </div>
      </section>
...
function Metric({
  ...
  return (
    <article className="border-border bg-card rounded-2xl border p-5">
      ...
      <p className="text-muted mt-4 text-xs font-bold">{label}</p>
