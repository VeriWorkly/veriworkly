...
export function NewDocumentModal({
  ...
  onImportAction?: (provider: "linkedin" | "github") => void;
}) {
  if (!open) return null;
  ...
  return (
    <Modal open={open} onClose={onClose}>
      ...
      >
        ...
        <div className="border-t border-border/70 p-4 sm:p-5 bg-card/10">
          <p className="text-accent text-[10px] font-bold tracking-[0.2em] uppercase mb-3">Import from profile</p>
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              className="border-border bg-background hover:border-accent/40 hover:bg-card flex items-center justify-center gap-2 rounded-xl border p-3 text-xs font-semibold transition outline-none"
              onClick={() => {
                onClose();
                onImportAction?.("linkedin");
              }}
            >
