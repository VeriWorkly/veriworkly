...
export function MonetizationActions({
  ...
  const update = async (nextStatus: "APPROVED" | "REJECTED" | "PAID") => {
    ...
    try {
      await fetchApiData(`/admin/monetization/withdrawals/${withdrawalId}`, {
        method: "PATCH",
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      {status === "APPROVED" ? (
        <Button size="sm" loading={loading === "PAID"} onClick={() => void update("PAID")}>
          Mark paid
        </Button>
      ) : null}
    </div>
  ...
  const submit = async () => {
    ...
    try {
      await fetchApiData(`/admin/monetization/${kind}`, {
        ...
        body: JSON.stringify(
          kind === "credits"
            ? { userId, amount: Number(value), reason }
            : { userId, key: value, reason },
        ),
      });
      setMessage("Saved.");
      router.refresh();
    ...
    <div className="space-y-3">
      ...
      {kind === "credits" ? (
        ...
        <select
          ...
          onChange={(e) => setValue(e.target.value)}
        >
          <option value="portfolio_publish">Portfolio publish</option>
          <option value="ai_credits">AI credits</option>
          <option value="custom_subdomain">Custom subdomain</option>
          <option value="analytics">Analytics</option>
          <option value="seo_controls">SEO controls</option>
          <option value="watermark_removal">Watermark removal</option>
        </select>
      )}
      <input
        className="border-border bg-background h-10 w-full rounded-lg border px-3 text-sm"
      ...
      />
      {message ? <p className="text-muted text-xs">{message}</p> : null}
      <Button onClick={() => void submit()}>Apply {kind}</Button>
    </div>
  );
}

export function AffiliateModeration({
  userId,
  status,
  tier,
}: {
  ...
  const save = async () => {
    await fetchApiData(`/admin/monetization/affiliates/${userId}`, {
      method: "PATCH",
      body: JSON.stringify({ status: nextStatus, tier: nextTier }),
    });
    router.refresh();
  };
  return (
...
export function CommissionForm() {
  ...
  const submit = async () => {
    try {
      await fetchApiData("/admin/monetization/commissions", {
        method: "POST",
        body: JSON.stringify({
          referredUserId,
          providerPaymentId,
          purchaseAmountCents: Math.round(Number(amount) * 100),
          status: "AVAILABLE",
        }),
      });
      setMessage("Commission created.");
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Failed.");
    }
  ...
  return (
    <div className="space-y-3">
      ...
      />
      {message ? <p className="text-muted text-xs">{message}</p> : null}
      <Button onClick={() => void submit()}>Create commission</Button>
    </div>
  );
}
...
export function CommissionActions({ id, status }: { id: string; status: string }) {
  ...
  const update = async (nextStatus: "AVAILABLE" | "REVERSED") => {
    await fetchApiData(`/admin/monetization/commissions/${id}`, {
      method: "PATCH",
      body: JSON.stringify({ status: nextStatus }),
    });
    router.refresh();
  ...
  return (
    <div className="flex gap-2">
      <Button size="sm" onClick={() => void update("AVAILABLE")}>
        Release
      </Button>
      <Button size="sm" variant="secondary" onClick={() => void update("REVERSED")}>
        Reverse
      </Button>
    </div>
  );
