  ...
  return (
    <section className="bg-ink-2 py-24 text-white md:py-32">
      <div className={`${templatesShell} grid gap-10 lg:grid-cols-[0.8fr_1.2fr]`}>
        <div>
          <p className="mb-5 text-[0.72rem] font-bold tracking-[0.16em] text-[#93c5fd] uppercase">
            Best fit
          </p>
          <h2 className="max-w-2xl text-[clamp(3rem,6vw,6rem)] leading-[0.88] font-bold tracking-[-0.08em]">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {details.bestFor.map((item) => (
            <div
              className="rounded-3xl border border-white/10 bg-white/4 p-5 text-sm leading-6 font-bold"
              key={item}
            >
              {item}
            </div>
          ))}
          <div className="bg-accent border-accent rounded-3xl border p-5 text-sm leading-6 font-bold sm:col-span-2">
