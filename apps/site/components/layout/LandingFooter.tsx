            ...
            <div className="flex items-center gap-1 rounded-full border border-black/10 bg-white/40 p-1.5 shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-black/20">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="rounded-full px-5 py-2 text-sm font-medium text-gray-600 transition-all duration-300 ease-out hover:bg-black/5 hover:text-gray-900 active:scale-95 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-white"
                >
                  {link.name}
                </Link>
              ))}
