    ...
    <section className="border-border/40 space-y-10 border-t pt-16">
      <div className="space-y-2 text-left">
        ...
        <p className="text-muted max-w-2xl text-xs leading-relaxed sm:text-sm">
          Common questions about how resume parsing works, scoring methods, keyword match
          calculations, and privacy boundaries.
        </p>
      </div>

      <Reveal>
        <Accordion type="single" collapsible defaultValue={faqs[0]?.id} className="gap-3.5">
          {faqs.map((faq) => (
            <AccordionItem
              key={faq.id}
              value={faq.id}
            ...
            >
              <AccordionTrigger className="px-5 py-4.5 text-sm font-bold tracking-tight sm:text-base">
                {faq.question}
              </AccordionTrigger>
              <AccordionContent className="text-muted px-5 py-4 text-xs leading-relaxed sm:text-sm">
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
