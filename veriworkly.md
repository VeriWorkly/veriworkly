...
- **Core scan** — a deterministic, non-AI, free-of-AI-cost rules engine: word count, presence of email/phone/links, Experience/Education/Skills as real headings on their own line, the share of bullets opening with an action verb, the share carrying a metric, length, and format risk. Two format checks read the uploaded PDF's geometry rather than its text — the share of lines split across a column gutter, and ruled table grids traced from the page's drawing operators — and are omitted from the report entirely when a resume arrives as pasted text with no geometry to measure. Produces a readiness score normalised over the applicable rules, and, if a job description is pasted in, a keyword-match score that segments the posting (dropping about-us/benefits/EEO copy), weights requirements above nice-to-haves and skills above ordinary English, treats "Go or Java" as one choice, and credits skills the resume evidences under another name. **Important nuance:** this core scan and the AI-powered analysis below draw from the _same_ scan-quota bucket, not separate meters — running the free core scan and then the AI analysis on the same resume consumes two scans against one shared quota, not one scan against each of two independent limits.
